...
def test_unhashed_args_state():
    ...
    one_arg = BuildConfig(
        files=FilePolicy.Nothing, unhashed_build_args={"Test": "Hi"}
    ).get_hash()
    two_args = BuildConfig(
        files=FilePolicy.Nothing, unhashed_build_args={"A": "B", "X": "Y"}
    ).get_hash()
    hash_arg = BuildConfig(
        files=FilePolicy.Nothing, build_args={"A": "B", "X": "Y"}
    ).get_hash()
